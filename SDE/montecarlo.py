...
def mc_pricing(
    ...
    **kwargs,
):  # kwargs used to allow to call function with more arguments which will not be use
    ...
    random_simulations = underlying.generate_random_values_in_required_format(
        nb_periods=1
    )(
        nb_simulations
    )  # [[generate_n_gaussian(underlying.dimension) for _ in exercise_times] for i in range(nb_simulations)]

    if quasi_random:
        random_simulations = underlying.generate_random_values_in_required_format(
            nb_periods=1, is_quasi=True
        )(
            nb_simulations
        )  # generate_n_gaussian_quasi_paths(nb_asset, nb_simulations, len(exercise_times))

    ############## APPLY ALGORITHM ##############
    actualised_payoff = lambda x: np.exp(
    ...
    if A_transformation:
        ...
        random_simulations_A_shift = apply_transformation_recursive(
            random_simulations, A_transformation
        )
        result_vec_A = [
            h_function(simulation_A) for simulation_A in random_simulations_A_shift
        ]
    ...
    if h0_function:
        ...
        def compute_almost_control_estimator(d_gaussian_vec, h_vector, m):
            ...
            def b_function(h0_vector, h_vector):
                ...
                up_vec = [
                    (i_h0 - m) * (i_h - h_bar) for i_h0, i_h in zip(h0_vector, h_vector)
                ]
            ...
            h0_vector = np.array(
                [h0_function(d_gaussian) for d_gaussian in d_gaussian_vec]
            )
        ...
        almost_controle_var_vec = compute_almost_control_estimator(
            random_simulations, result_vec, m_value
        )

        if A_transformation:  # applying antithetic to control
            almost_controle_var_vec_A = compute_almost_control_estimator(
                random_simulations_A_shift, result_vec_A, m_value
            )
            almost_controle_var_vec = 0.5 * (
                almost_controle_var_vec + almost_controle_var_vec_A
            )
    ...
    if display_performance:
        extra_info = ""
        if A_transformation:
            extra_info += "Antithetic|"
        if h0_function:
            extra_info += "Controle var.|"
        if quasi_random:
            extra_info += "Quasi.|"
...
def mcls_pricing(
    ...
    **kwargs,
    ...
    def LS_algorithm_applied_to_gaussians(d_gaussian_path_vec):
        ...
        simulations_vec = [
            underlying.simulate_path_with_brownian(exercise_times, brownians)
            for brownians in d_gaussian_path_vec
        ]
        ...
        payoff_vec = np.array(
            [
                np.exp(-actualisation_rate * (exercise_times[-1] - exercise_times[-2]))
                * terminal_payoff_function(S_T)
                for S_T in S_T_vec
            ]
        )  # actualise payoff needed in first iteration
        ...
        for i_period in range(nb_periods - 2, 0, -1):
            ...
            itm_index = [
                i
                for i, value in enumerate(asset_value_period)
                if terminal_payoff_function(value) > 0
            ]

            if len(itm_index) == 0:  # case where no path is ITM -> will lead to error
                payoff_vec *= np.exp(
                    -actualisation_rate
                    * (exercise_times[i_period] - exercise_times[i_period - 1])
                )
                continue
            ...
            ## run regression ##
            print_log_info(
                f"Period {round(exercise_times[i_period], 2)}: perform LS", verbose
            )
            ...
            print_log_info(
                f"Period {round(exercise_times[i_period], 2)}: compute optimal decision",
                verbose,
            )
            ...
            for i in tqdm(range(nb_simulations), disable=not verbose):
                if i in itm_index:
                    if (
                        terminal_payoff_function(asset_itm_vec[cpt])
                        > continuation_vec[cpt]
                    ):
    ...
    assert is_sorted_ascending(
        exercise_times
    ), "exercise periods must be chronologically sorted"
    ...
    if quasi_random:
        random_simulations = underlying.generate_random_values_in_required_format(
            nb_periods=len(exercise_times, is_quasi=True)
        )(
            nb_simulations
        )  # generate_n_gaussian_quasi_paths(nb_asset, nb_simulations, len(exercise_times))
    ...
    if h0_function:
        ...
        def compute_almost_control_estimator(d_gaussian_path_vec, h_vector, m):
            ...
            h0_vector = np.array(
                [
                    h0_function(d_gaussian_path)
                    for d_gaussian_path in d_gaussian_path_vec
                ]
            )
