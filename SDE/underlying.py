...
from SDE.utils.random_generation import generate_n_gaussian, generate_n_gaussian_quasi_paths, \
    generate_n_gaussian_quasi  # type: ignore
...
class Underlying:
    """ Base class for all underlying assets to model.

    Attributes:

    - initial_spot (Union[List[float],float]): initial spot in 1 or d-dimension
    ...
    - price(self, actualisation_rate: float, maturity: float, strike: float, option_type=str): returns the price of the option for the given underlying
    - simulate_path(self, time_vec: List[float]): returns the path of the underlying for the given times

    """
    def __init__(self, initial_spot: Union[List[float], float], dimension: int, live_seed=True):
        self.initial_spot = initial_spot
        self.dimension = dimension
        self.generator_function = lambda x: generate_n_gaussian(x, live_seed=live_seed)

    def price(self, actualisation_rate: float, maturity: float, strike: float, option_type=str) -> float:
        """
        Function that returns the price of the option given an underlying asset and a deterministic payoff function.

        :return: deterministic price of called payoff for asset
        """
    ...
    def simulate_path(self, time_vec: List[float]) -> List[float]:
        ...
        return self.simulate_path_with_brownian(time_vec,
                                                self.generate_random_values_in_required_format(len(time_vec))(1))
    ...
    def generate_random_values_in_required_format(self, nb_periods, is_quasi_random: bool = False) -> Callable[
        [int], float]:
        """
        ...
        assert hasattr(self,
                       "nb_brownians_per_sim"), "object must have an attribute representing the number of simulations required per simulation of asset"
        ...
        def generate_values(nb_simulations):
            ...
            if dimension == 1 and nb_brownians_per_sim == 1 and nb_periods == 1:
                ...
                simulations = np.array(simulations_in_list).reshape(nb_simulations, nb_brownians_per_sim,
                                                                    nb_periods).tolist()
                ...
                simulations = np.array(simulations_in_list).reshape(nb_simulations, nb_periods, dimension).tolist()
